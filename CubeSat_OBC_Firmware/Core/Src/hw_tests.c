/**
 * @file    hw_tests.c
 * @brief   Peripheral bring-up tests for the CubeSat OBC board.
 */
#include "hw_tests.h"
                       ...
                       const char *name)
{
    ...
    const uint32_t end = start + OBC_LOOPBACK_DURATION_MS;

    Console_Printf("Starting %s loopback for %u ms\r\n", name, OBC_LOOPBACK_DURATION_MS);
...
void HwTest_UartEcho(UART_HandleTypeDef *huart, const char *port_name)
{
    ...
    status = HAL_UART_Receive(huart, &received, 1U, OBC_UART_TEST_TIMEOUT_MS);
