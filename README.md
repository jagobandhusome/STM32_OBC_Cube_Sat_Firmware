...
[![Firmware](https://img.shields.io/badge/firmware-1.1.0-green)](SOARRSO_CubeSat_OBC_Firmware)
[![IDE](https://img.shields.io/badge/IDE-STM32CubeIDE-lightgrey)](https://www.st.com/en/development-tools/stm32cubeide.html)

## Short description

SOARRSO CubeSat OBC is an STM32H753ZI on-board computer project with CubeSat-style flight software (command, housekeeping, SAFE/NOMINAL/TEST modes), board tests, and hardware design files.

## Topics / hashtags

Use these as GitHub **Topics** (About → Topics) or post tags:

`#SOARRSO` `#CubeSat` `#OBC` `#OnBoardComputer` `#STM32` `#STM32H7` `#STM32H753` `#FlightSoftware` `#Housekeeping` `#Telecommand` `#Telemetry` `#SpaceSystems` `#EmbeddedSystems` `#CANBus` `#FDCAN` `#Satellite` `#RemoteSensing` `#SPARRSO`

GitHub topic names (no `#`):

```text
soarrso, cubesat, obc, stm32, stm32h7, stm32h753, flight-software, housekeeping,
...
| Path | Contents |
| --- | --- |
| [`SOARRSO_CubeSat_OBC_Firmware/`](SOARRSO_CubeSat_OBC_Firmware) | STM32CubeIDE project and flight / test firmware |
| [`SOARRSO_CubeSat_OBC_Hardware/`](SOARRSO_CubeSat_OBC_Hardware) | Schematic, PCB, BOM, Gerbers |
| [`SOARRSO_CubeSat_OBC_Assets/`](SOARRSO_CubeSat_OBC_Assets) | Console and setup images |
...
2. Open `SOARRSO_CubeSat_OBC_Firmware` in **STM32CubeIDE**.
3. Build **Debug** and flash with ST-LINK.
4. Serial terminal: **115200 8-N-1** on the ST-LINK VCP.
5. Type `help` or `hk` and press Enter.

Firmware details: [`SOARRSO_CubeSat_OBC_Firmware/README.md`](SOARRSO_CubeSat_OBC_Firmware/README.md).
...
- [Schematic (PDF)](SOARRSO_CubeSat_OBC_Hardware/SOARRSO_CubeSat_OBC_Schematic.pdf)
- [BOM](SOARRSO_CubeSat_OBC_Hardware/SOARRSO_CubeSat_OBC_BOM.xlsx)
- [PCB CAD](SOARRSO_CubeSat_OBC_Hardware/CAD/SOARRSO_CubeSat_OBC_PCB.brd)
- [Gerbers](SOARRSO_CubeSat_OBC_Hardware/Gerbers/SOARRSO_CubeSat_OBC_Gerber.zip)
...
- Application firmware: use and modify for the SOARRSO CubeSat OBC project.
