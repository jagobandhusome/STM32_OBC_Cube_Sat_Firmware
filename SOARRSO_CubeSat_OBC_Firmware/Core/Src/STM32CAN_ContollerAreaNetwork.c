 ...
 * @brief Classic CAN 2.0 driver used by the SOARRSO CubeSat OBC.
 */
...
void EH_Exception(char* sFile, char* sLine, char* sErrorString)
{
	...
	{
		const size_t length = strlen(sErrorString);
		(void)HAL_UART_Transmit(&huart3, (uint8_t *)sErrorString, (uint16_t)length, 1000U);
	}
}
